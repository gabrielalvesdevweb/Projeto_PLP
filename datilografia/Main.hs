...
menu = do
  ...
  if opcao == 1 then do
    ...
    if(userName `elem` usuarios) then do
      putStrLn "Esse nome de usuário já foi utilizado anteriormente! \nEfetue login ou registre-se com outro nome. \n" 
      menu
    else do
...
controlaNivelFacil nvl =
  if nvl == 1 then readFile "Textos/facil1.txt"
  else if nvl == 2 then readFile "Textos/facil2.txt"
  else readFile "Textos/facil3.txt"

controlaNivelMedio :: Int -> IO String
controlaNivelMedio nvl =
  if nvl == 1 then readFile "Textos/medio1.txt"
  else if nvl == 2 then readFile "Textos/medio2.txt"
  else readFile "Textos/medio3.txt"

controlaNivelDificil :: Int -> IO String
controlaNivelDificil nvl =
  if nvl == 1 then readFile "Textos/dificil1.txt"
  else if nvl == 2 then readFile "Textos/dificil2.txt"
  else readFile "Textos/dificil3.txt"
